...
//    * 회원정보 수정
//    *
//    * 변경한 항목만 PATCH Body에 포함합니다.
//    *
//    * 비밀번호 검증 Token은 사용하지 않습니다.
//    * 회원정보 수정 요청에는 Access Token만 전달합니다.
//    */
//   const handleSubmit = async (event) => {
//     event.preventDefault();
...
//     if (password && !isValidPassword(password)) {
//       showToast(
//         "비밀번호는 8자 이상 16자 이하로 영문 대문자, 소문자, 숫자, 특수문자(@ $ ! % * # ? &)를 각각 하나 이상 포함해야 합니다.",
//         false,
//       );
...
//       /*
//        * 변경된 회원정보 + Access Token
//        *
//        * passwordVerificationToken은
//        * 더 이상 사용하지 않습니다.
//        */
//       const result = await updateMe(updateData, accessToken);
...
//    * 이 함수가 실행됩니다.
//    *
//    * 별도의 passwordVerificationToken은
//    * 사용하지 않습니다.
//    */
//   const handleDeleteAccount = async () => {
//     if (isDeleting) {
...
//       /*
//        * PasswordConfirmModal에서
//        * 비밀번호 확인이 성공한 뒤 실행됩니다.
//        *
//        * 여기서는 실제 회원탈퇴 API만 호출합니다.
//        */
//       const result = await deleteMe(accessToken);
...
//       /*
//        * 탈퇴 완료 후 메인 페이지로 이동합니다.
//        *
//        * logout()을 호출하지 않습니다.
//        * 이미 서버에서 회원 계정이 삭제된 상태이므로
//        * 별도의 로그아웃 API 요청을 보내지 않습니다.
//        */
//       navigate("/", {
//         replace: true,
//       });
...
const UpdateInfo = () => {
  ...
  /*
   * 탈퇴 성공 직후 clearAuth/logout을 실행하지 않습니다.
   *
   * 회원탈퇴 API 성공
   * → 안내 토스트
   * → 메인페이지 이동
   ...
   *
   * PasswordConfirmModal에서
   * 현재 비밀번호 확인이 성공한 뒤 실행됩니다.
   *
   * 중요:
   * 회원탈퇴 성공 후 logout()을 호출하지 않습니다.
   *
   * DELETE /users/me
   * → 성공
   * → 모달 닫기
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      /*
       * logout() / clearAuth()를 여기서 실행하지 않습니다.
       *
       * 탈퇴 성공 직후 인증 상태를 먼저 변경하면
       * 인증 라우팅이 반응하여 메인페이지 이동 전에
       * 로그인 페이지로 이동할 수 있기 때문입니다.
       */
      navigate("/", {
        replace: true,
      });
